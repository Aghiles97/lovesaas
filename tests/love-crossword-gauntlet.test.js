// tests/love-crossword-gauntlet.test.js
// 7-Gate Automated Verification Suite for love_crossword Widget
...
const WIDGET_ID = "love_crossword";
...
console.log("\n=============================================");
console.log("🚀 LOVE CROSSWORD 7-GATE GAUNTLET RUNNER");
console.log("=============================================\n");
...
try {
  ...
  if (typeof renderer === "function") {
    const mockData = {
      title: "Our Crossword",
      words: [
        { id: "A1", dir: "across", num: 1, word: "TEST", clue: "Test clue", r: 0, c: 0 }
      ]
    };
    const mockRoot = { partner1: "Alex", partner2: "Jordan" };
    ...
    assert(renderedHtml.includes("crosswordGrid"), "Rendered HTML contains #crosswordGrid element");
  }
} catch (e) {
...
const cwDefault = DEFAULT_SECTIONS_DATA[WIDGET_ID];
assert(cwDefault && Array.isArray(cwDefault.words), "DEFAULT_SECTIONS_DATA has words array for " + WIDGET_ID);
assert(cwDefault.words.length === 10, "Default crossword has exactly 10 words (found " + cwDefault.words.length + ")");

// Verify intersections mathematically
const cellOccupants = {};
let collisionError = null;
cwDefault.words.forEach(w => {
  for (let i = 0; i < w.word.length; i++) {
    const r = w.dir === "across" ? w.r : w.r + i;
    const c = w.dir === "across" ? w.c + i : w.c;
    const char = w.word[i];
    const key = `${r},${c}`;
    if (cellOccupants[key] && cellOccupants[key] !== char) {
      collisionError = `Collision at (${r},${c}): existing '${cellOccupants[key]}' vs new '${char}' from ${w.id}`;
    }
    cellOccupants[key] = char;
  }
});
assert(!collisionError, "All word intersections are mathematically consistent: " + (collisionError || "100% Match"));
